    ...
    <div class="page-wrapper">
			<div class="page-content">
				...
				<div style="display: flex; justify-content: space-between">
					<h6 class="mb-0 text-uppercase">Datatable Visit</h6>
					<a href="{{ route('visit.create') }}" class="btn btn-success px-2" style="justify-content: space-between ">
								...
								<tbody>
									...
									@forelse ($data as $item)
										<tr>
											...
											<td>{{ $item->act_date }}</td>
											<td><img src="{{ url('visit_photo/'.$item->photo_url) }}" width='100px' height="100px"/></td>
											<td>{{ $item->description }}</td>
											<td>
												<form action="{{ route('visit.destroy', $item->id) }}" method="post">
													@csrf
													@method('delete')
													<a id="berkasId" href="{{ route('visit.edit', $item->id) }}" class="btn btn-warning">
													...
													<button type="button" 
														onclick="getElementById('gambarModal').src = `{{asset($item->photo_url != '' ? 'visit_photo/'.$item->photo_url : 'no-pictures.png')}}`" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleVerticallycenteredModal">
			...
			<div class="modal-dialog modal-dialog-centered modal-lg">
				<div class="modal-content" >
					<div class="modal-header">
						<h5 class="modal-title">Foto Visit</h5>
